...
module Poker
  describe HandEvaluator do
    ...
    def expect_higher(higher_string, lower_string)
      ...
      expect(HandEvaluator.new.return_stronger_hand(higher, lower)).to(
        eq(higher)
      )

      expect(HandEvaluator.new.return_stronger_hand(lower, higher)).to(
        eq(higher)
      )
    end
  end
